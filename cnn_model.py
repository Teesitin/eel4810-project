...
def build_raw_windows(df_split, base_features, window, split_name, report_lines):
    ...
    log_print(
        report_lines,
        f"{split_name}: rows={len(df_split)}, built_windows={built}, skipped_neutral={skipped_neutral}"
    )
...
def train_only_clip_scale_3d(X_train, X_other):
    ...
    clip_lower = np.quantile(flat_tr, CLIP_LOWER_Q, axis=0)
    clip_upper = np.quantile(flat_tr, CLIP_UPPER_Q, axis=0)

    X_train = np.clip(X_train, clip_lower, clip_upper)
    X_other = np.clip(X_other, clip_lower, clip_upper)

    flat_tr2 = X_train.reshape(-1, F)
    train_mean = flat_tr2.mean(axis=0)
    train_std = flat_tr2.std(axis=0)
...
class StockCNN(nn.Module):
    def __init__(self, n_features, conv_channels, kernel_sizes, fc_dims, dropout):
        super().__init__()
        assert len(conv_channels) == len(kernel_sizes), "conv_channels and kernel_sizes must have the same length"
...
log_print(report_lines, f"Raw test shape:     {X_test.shape}, {y_test.shape}")
...
for fold_idx, (cv_train_idx, cv_val_idx) in enumerate(tscv.split(X_trainval), start=1):
    ...
    X_inner_train_scaled, X_stop_scaled, clip_lower, clip_upper, train_mean, train_std = train_only_clip_scale_3d(
...
X_final_train_scaled, X_final_stop_scaled, clip_lower, clip_upper, train_mean, train_std = train_only_clip_scale_3d(
...
for seed in stable_seeds:
    ...
    first_conv_w = m.conv_block[0].weight.detach().cpu().abs()
    importance = first_conv_w.mean(dim=(0, 2)).numpy()
...
if test_metrics["accuracy"] >= 0.52:
    notes.append("Test accuracy cleared 52%. CNN is a viable comparison point against the MLP baseline.")
elif test_metrics["accuracy"] >= 0.50:
    notes.append("Above coin-flip territory, but edge is modest. Try reducing WINDOW or PREDICTION_HORIZON.")
else:
    notes.append("Below target. Levers: TRAIN_MOVE_QUANTILE=0.50, WINDOW=60, PREDICTION_HORIZON=30, or add a residual connection.")

if test_metrics["roc_auc"] < 0.45 and flipped_auc > 0.55:
    notes.append("Warning: test ranking appears directionally inverted. Check regime shift and label stability.")

notes.append(
    "CNN v2: smaller model (32/64/128 channels, 1 FC layer), higher dropout (0.5), shorter window, "
    "stronger weight decay (5e-3). These all target overfitting."
)
notes.append(
    "If stop loss still diverges early, the regime shift is likely the main remaining problem. "
    "Next lever: train only on more recent years."
)
notes.append(
    "If results improve but plateau below 52%, try adding a residual skip connection from the input norm "
    "to the pooled output before the FC head."
)
