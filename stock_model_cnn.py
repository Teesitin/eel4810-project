...
MAX_EPOCHS = 50           # more epochs to compensate for slower learning at higher regularization
BATCH_SIZE = 256
MAX_LR = 2e-4             # was 3e-4 — lower LR pairs better with higher dropout
WEIGHT_DECAY = 5e-3       # was 1e-3 — stronger L2 penalty
DROPOUT = 0.5             # was 0.3 — main fix for overfitting
LABEL_SMOOTHING = 0.05    # was 0.02 — smoother targets reduce overconfidence
EARLY_STOPPING_PATIENCE = 8  # was 6 — give more room with slower convergence
NUM_WORKERS = 0
GRAD_CLIP_NORM = 1.0
...
# CNN architecture — deliberately smaller than v1 to fight overfitting.
# Fold 1 in v1 had ~3k training samples vs 175k params — that's ~58x overparameterized.
# These settings give ~30k params, a much healthier ratio.
CONV_CHANNELS   = [32, 64, 128]   # was [64, 128, 256]
KERNEL_SIZES    = [5,  3,   3]    # unchanged
FC_DIMS         = [64]            # was [128, 64] — one layer is enough given smaller backbone

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

# Identical feature set to MLP for a fair comparison
BASE_FEATURES = [
    "return_1",
    "return_5",
    "return_15",
...
# ============================================================
# CNN-SPECIFIC: raw window builder
# Returns X of shape (N, WINDOW, n_features) — NOT flattened.
# The CNN reads the full temporal sequence; no summary compression.
# ============================================================
def build_raw_windows(df_split, base_features, window, split_name, report_lines):
    X, y, times, future_ret = [], [], [], []
    target_values    = df_split["target"].values
    feat_values      = df_split[base_features].values.astype(np.float32)
    times_values     = df_split["timestamp"].values
    future_ret_values = df_split["future_return"].values

    skipped_neutral = 0
    built = 0

    for i in range(window, len(df_split)):
        label = target_values[i]
        if np.isnan(label):
            skipped_neutral += 1
            continue

        arr = feat_values[i - window : i]   # shape: (window, n_features)
        X.append(arr)
        y.append(label)
        times.append(times_values[i])
        future_ret.append(future_ret_values[i])
        built += 1
    ...
    log_print(report_lines,
              f"{split_name}: rows={len(df_split)}, built_windows={built}, "
              f"skipped_neutral={skipped_neutral}")
    return X, y, times, future_ret


# ============================================================
# SCALING — operates on the feature axis (axis=-1) so the same
# per-feature statistics are applied across every timestep.
# X shape: (N, window, n_features)
# ============================================================
def train_only_clip_scale_3d(X_train, X_other):
    # Flatten time dimension to compute per-feature quantiles
    N_tr, T, F = X_train.shape
    flat_tr = X_train.reshape(-1, F)

    clip_lower = np.quantile(flat_tr, CLIP_LOWER_Q, axis=0)   # (F,)
    clip_upper = np.quantile(flat_tr, CLIP_UPPER_Q, axis=0)   # (F,)

    X_train = np.clip(X_train, clip_lower, clip_upper)
    X_other = np.clip(X_other, clip_lower, clip_upper)

    flat_tr2   = X_train.reshape(-1, F)
    train_mean = flat_tr2.mean(axis=0)           # (F,)
    train_std  = flat_tr2.std(axis=0)            # (F,)
...
def transform_with_existing_stats_3d(X, clip_lower, clip_upper, train_mean, train_std):
...
def find_best_threshold(y_true, y_prob):
    ...
    search_df = search_df.sort_values(
        ["balanced_accuracy", "accuracy", "f1"], ascending=False
    )
...
class StockCNN(nn.Module):
    """
    1-D convolutional model for sequential OHLCV features.

    Input:  (batch, timesteps, n_features)
    Conv1d expects (batch, channels, length), so we permute first.

    Architecture:
        - Per-feature layer norm on the raw input
        - Stack of Conv1d → GELU → Dropout blocks with increasing channels
        - AdaptiveAvgPool1d collapses the time dimension to 1
        - Small FC head → scalar logit
    """
    def __init__(self, n_features, conv_channels, kernel_sizes, fc_dims, dropout):
        super().__init__()
        ...
        for out_ch, ks in zip(conv_channels, kernel_sizes):
            padding = ks // 2           # 'same' padding — output length == input length
            conv_layers.extend([
        ...
        fc_layers = [nn.Flatten()]                     # → (batch, last_ch)
    ...
    def forward(self, x):
        ...
        x = self.conv_block(x)          # → (batch, last_ch, 1)
        return self.head(x).squeeze(1)  # → (batch,)


# ============================================================
# TRAIN / EVAL  (identical contract to MLP file)
# ============================================================
...
def train_one_model(seed, X_train, y_train, X_stop, y_stop,
                    device, tag, report_lines, history_rows, verbose=True):
    set_seed(seed)
    ...
    n_features = X_train.shape[2]      # (N, window, n_features)
    model = StockCNN(
    ...
    if verbose:
        ...
        log_print(report_lines,
                  f"Input shape: (batch, {WINDOW}, {n_features})  "
                  f"→ conv channels: {CONV_CHANNELS}  kernels: {KERNEL_SIZES}  "
                  f"fc: {FC_DIMS}")
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        improved = (stop_auc > best_stop_auc + 1e-4) or (
            abs(stop_auc - best_stop_auc) <= 1e-4
            and stop_bal_acc > best_stop_bal_acc + 1e-4
        )
        ...
        if epochs_no_improve >= EARLY_STOPPING_PATIENCE:
            if verbose:
                log_print(report_lines,
                          f"Early stopping for {tag} | seed={seed} at epoch {epoch}")
            break
...
log_print(report_lines, f"Model type: 1-D CNN (Conv1d) on raw sequential windows — v2 (anti-overfit)")
log_print(report_lines, f"--- v2 changes vs v1 ---")
log_print(report_lines, f"  WINDOW:         120 → 60    (fewer timesteps = smaller overfit surface)")
log_print(report_lines, f"  DROPOUT:        0.3 → 0.5   (primary regularization fix)")
log_print(report_lines, f"  WEIGHT_DECAY:   1e-3 → 5e-3 (stronger L2)")
log_print(report_lines, f"  CONV_CHANNELS:  [64,128,256] → [32,64,128]  (model ~6x smaller)")
log_print(report_lines, f"  FC_DIMS:        [128,64] → [64]             (shallower head)")
log_print(report_lines, f"  LABEL_SMOOTHING:0.02 → 0.05  (reduce overconfidence)")
log_print(report_lines, f"  MAX_LR:         3e-4 → 2e-4  (pairs with higher dropout)")
log_print(report_lines, f"  MAX_EPOCHS:     35 → 50      (more room at slower convergence)")
log_print(report_lines, f"  ES_PATIENCE:    6 → 8        (matches slower convergence)")
log_print(report_lines, f"------------------------")
...
log_print(report_lines, f"CNN conv channels: {CONV_CHANNELS}")
log_print(report_lines, f"CNN kernel sizes:  {KERNEL_SIZES}")
log_print(report_lines, f"CNN fc dims:       {FC_DIMS}")
...
log_print(report_lines, f"Threshold search range: [{THRESHOLD_MIN}, {THRESHOLD_MAX}] "
                        f"with {THRESHOLD_STEPS} steps")
...
log_print(report_lines,
          f"Chosen move threshold (absolute future return): {move_threshold:.6f}")
...
# ============================================================
section(report_lines, "5. BUILD RAW WINDOWS")
# ============================================================
# CNN key difference: raw (WINDOW, n_features) tensors — no summary compression
X_trainval, y_trainval, time_trainval, future_ret_trainval = build_raw_windows(
    df_trainval_rows, BASE_FEATURES, WINDOW, "Trainval", report_lines
)
X_test, y_test, time_test, future_ret_test = build_raw_windows(
    df_test_rows, BASE_FEATURES, WINDOW, "Test", report_lines
)

log_print(report_lines,
          f"\nRaw trainval shape: {X_trainval.shape}, {y_trainval.shape}")
log_print(report_lines,
          f"Raw test shape:     {X_test.shape}, {y_test.shape}")
...
oof_probs_by_seed = {
    seed: np.full(len(X_trainval), np.nan, dtype=np.float32)
    for seed in ENSEMBLE_SEEDS
}
fold_results = []

for fold_idx, (cv_train_idx, cv_val_idx) in enumerate(
        tscv.split(X_trainval), start=1):
    ...
    (X_inner_train_scaled, X_stop_scaled,
     clip_lower, clip_upper, train_mean, train_std) = train_only_clip_scale_3d(
        X_inner_train, X_stop
    )
    X_val_fold_scaled = transform_with_existing_stats_3d(
        X_val_fold, clip_lower, clip_upper, train_mean, train_std
    )

    log_print(report_lines,
              f"\nFold {fold_idx}: train={len(inner_train_idx)}, "
              f"stop={len(stop_idx)}, val={len(cv_val_idx)}")
    ...
    for seed in ENSEMBLE_SEEDS:
        ...
        fold_bal_acc = balanced_accuracy_score(y_val_fold,
...
for seed, probs in oof_probs_by_seed.items():
    ...
    seed_bal_acc = balanced_accuracy_score(y_trainval[mask],
...
stable_seeds = seed_scores_df[
    seed_scores_df["oof_auc"] >= MIN_SEED_OOF_AUC
]["seed"].tolist()
...
stable_oof_arrays = [oof_probs_by_seed[s] for s in stable_seeds]
...
log_print(report_lines,
          f"Best threshold from OOF validation: {best_threshold:.4f}")
...
(X_final_train_scaled, X_final_stop_scaled,
 clip_lower, clip_upper, train_mean, train_std) = train_only_clip_scale_3d(
    X_final_train, X_final_stop
)
X_test_scaled = transform_with_existing_stats_3d(
    X_test, clip_lower, clip_upper, train_mean, train_std
)
...
for seed in stable_seeds:
    ...
    log_print(report_lines,
              f"Final seed={seed} | best_epoch={best_epoch} | "
              f"best_stop_auc={best_stop_auc:.6f} | "
              f"best_stop_bal_acc={best_stop_bal_acc:.6f}")
...
flipped_auc  = roc_auc_score(test_targets, 1.0 - test_probs_ens)  # diagnostic only
...
log_print(report_lines,
          f"Diagnostic flipped ROC-AUC (do not tune on this): {flipped_auc:.6f}")
...
log_print(report_lines,
    f"Test probs mean={test_probs_ens.mean():.6f} "
    f"std={test_probs_ens.std():.6f} "
    f"min={test_probs_ens.min():.6f} "
    f"max={test_probs_ens.max():.6f}"
)

# ============================================================
section(report_lines, "10. FILTER IMPORTANCE (first-layer weight norms)")
# ============================================================
# For a CNN the direct analog to MLP first-layer importance is the L1
# norm of each input-channel's filter weights — tells us which features
# the first conv layer pays most attention to across all its filters.
member_importances = []
for seed in stable_seeds:
    checkpoint_path = os.path.join(RUNS_DIR, f"{run_name}_seed{seed}_best.pt")
    m = StockCNN(
    ...
    # First Conv1d: weight shape is (out_ch, in_ch, kernel)
    # Mean over output channels and kernel positions → (in_ch,) = (n_features,)
    first_conv_w = m.conv_block[0].weight.detach().cpu().abs()  # (out_ch, in_ch, ks)
    importance   = first_conv_w.mean(dim=(0, 2)).numpy()        # (in_ch,)
    member_importances.append(importance)

avg_importance = np.mean(np.stack(member_importances, axis=0), axis=0)
...
log_print(report_lines, "Top features by first-conv filter weight norm:")
log_print(report_lines, fi_df.to_string(index=False))
...
if test_metrics["accuracy"] >= 0.52:
    notes.append(
        "Test accuracy cleared 52%. CNN is a viable comparison point against the MLP baseline."
    )
elif test_metrics["accuracy"] >= 0.50:
    notes.append(
        "Above coin-flip territory, but edge is modest. "
        "Try reducing WINDOW or PREDICTION_HORIZON."
    )
else:
    notes.append(
        "Below target. Levers: TRAIN_MOVE_QUANTILE=0.50, WINDOW=60, "
        "PREDICTION_HORIZON=30, or add a residual connection."
    )

if test_metrics["roc_auc"] < 0.45 and flipped_auc > 0.55:
    notes.append(
        "Warning: test ranking appears directionally inverted. "
        "Check regime shift and label stability."
    )

notes.append(
    "CNN v2: smaller model (32/64/128 channels, 1 FC layer), higher dropout (0.5), "
    "shorter window (60 bars), stronger weight decay (5e-3). "
    "These all target the v1 overfitting signature: train loss 0.3 vs stop loss 4.0."
)
notes.append(
    "If stop loss still diverges early (epoch 3-4), the regime shift is the main remaining "
    "problem. Next lever: train only on data from 2023 onward to reduce distribution mismatch."
)
notes.append(
    "If results improve but plateau below 52%, try adding a residual skip connection "
    "from the input norm to the pooled output before the FC head."
)
