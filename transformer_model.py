...
ENSEMBLE_SEEDS = [42, 1337, 2026, 777, 31415, 123, 999, 5555, 8080, 2718]
...
class TransformerEncoderClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        # Many-to-one sequence classification: use the final time-step representation.
...
run_name = f"transformer_encoder_timesplit_sequence_w{WINDOW}_h{PREDICTION_HORIZON}_{run_timestamp}"
...
log_print(report_lines, f"D model: {D_MODEL}")
log_print(report_lines, f"Attention heads: {NHEAD}")
log_print(report_lines, f"Num encoder layers: {NUM_LAYERS}")
log_print(report_lines, f"Feedforward size: {DIM_FEEDFORWARD}")
...
log_print(report_lines, "Skipped for the current Transformer Encoder version.")
...
if test_metrics["accuracy"] >= 0.52:
    notes.append("Test accuracy cleared 52%. This configuration is worth keeping as a Transformer Encoder candidate")
...
notes.append("This Transformer file keeps full sequence windows and uses self-attention over the window instead of recurrent LSTM memory.")
...
notes.append("If this still saturates, the next levers are D_MODEL=64/128, NHEAD=4/8, WINDOW=90, TRAIN_MOVE_QUANTILE=0.50, or PREDICTION_HORIZON=30.")
