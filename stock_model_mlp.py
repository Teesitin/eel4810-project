...
def make_summary_feature_names(base_features):
    names = []
    for stat in SUMMARY_STATS:
        for feat in base_features:
            names.append(f"{feat}__{stat}")
    return names


def build_summary_windows(df_split, base_features, window, recent_window, split_name, report_lines):
    ...
    skipped_neutral = 0
    built = 0
    recent_window = min(recent_window, window)
    ...
    for i in range(window, len(df_split)):
        ...
        arr = feat_values[i - window:i]  # (window, features)
        ...
        maxv = arr.max(axis=0)
        delta = arr[-1] - arr[0]
        recent_minus_mean = arr[-recent_window:].mean(axis=0) - mean
    ...
    log_print(report_lines, f"{split_name}: rows={len(df_split)}, built_windows={built}, skipped_neutral={skipped_neutral}")
...
def train_one_model(seed, X_train, y_train, X_stop, y_stop, device, tag, report_lines, history_rows, verbose=True):
    ...
    model = StableMLP(input_dim=input_dim, hidden_dims=HIDDEN_DIMS, dropout=DROPOUT).to(device)
...
log_print(report_lines, f"Summary stats ({len(SUMMARY_STATS)}): {SUMMARY_STATS}")
log_print(report_lines, f"Summary feature count: {len(BASE_FEATURES) * len(SUMMARY_STATS)}")
log_print(report_lines, f"CV splits: {CV_SPLITS}")
log_print(report_lines, f"CV gap: {CV_GAP}")
log_print(report_lines, f"Recent summary window: {RECENT_SUMMARY_WINDOW}")
...
log_print(report_lines, f"Hidden dims: {HIDDEN_DIMS}")
...
section(report_lines, "5. BUILD SUMMARY WINDOWS")
X_trainval, y_trainval, time_trainval, future_ret_trainval = build_summary_windows(
    df_trainval_rows,
    BASE_FEATURES,
    WINDOW,
    RECENT_SUMMARY_WINDOW,
    "Trainval",
    report_lines,
)
X_test, y_test, time_test, future_ret_test = build_summary_windows(
    df_test_rows,
    BASE_FEATURES,
    WINDOW,
    RECENT_SUMMARY_WINDOW,
...
summary_feature_names = make_summary_feature_names(BASE_FEATURES)

log_print(report_lines, f"\nSummary trainval shape: {X_trainval.shape}, {y_trainval.shape}")
log_print(report_lines, f"Summary test shape:     {X_test.shape}, {y_test.shape}")
...
for seed in stable_seeds:
    ...
    first_layer_abs = model.backbone[0].weight.detach().cpu().abs().mean(dim=0).numpy()
    member_importances.append(first_layer_abs)

avg_importance = np.mean(np.stack(member_importances, axis=0), axis=0)

fi_df = pd.DataFrame({
    "feature": summary_feature_names,
    "importance": avg_importance,
}).sort_values("importance", ascending=False)

log_print(report_lines, "Top 40 first-layer input importances:")
log_print(report_lines, fi_df.head(40).to_string(index=False))
...
if test_metrics["accuracy"] >= 0.52:
    notes.append("Test accuracy cleared 52%. This configuration is worth keeping as the main MLP baseline.")
...
notes.append("This file now avoids flattening every timestep directly and instead feeds summary features into the MLP, which is the main anti-overfitting change.")
...
notes.append("If this still saturates, the next stronger comparison should be a GRU or LSTM on the exact same target and summary/feature logic.")
